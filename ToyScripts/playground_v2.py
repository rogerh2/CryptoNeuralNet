...
def get_nth_hr_block(test_data, start_time, n=48, time_unit='hours'):
    ...
    data_block = test_data[(test_data.index >= start_time) & (test_data.index <= start_date_time + time_delta)]
    return data_block
...
def find_trades(data_block, trade_data_frame=None, n=24, m=4, max_iterations=5, time_unit='hours'):
    ...
    buy_x_point, buy_y_point, n_buy = buy_trade_loop(buy_x_point, buy_y_point, 1, max_iterations, data_block, time_delta, data_block.index[0] + time_delta)
...
test_data = DataSet("2018-04-25 18:00:00 EST", "2018-04-30 18:00:00 EST", days=1, google_list=['Etherium'], prediction_ticker='ltc', bitinfo_list=['btc', 'eth', 'ltc'], time_units='hours')
