...
from CryptoPredict.CryptoPredict import CryptoCompare as crypcomp
...
class TriangularArbitridgeBot(sb.Bot):
    ...
    def score_step(self, base_ticker, quote_ticker, value_ticker='USD'):
        t0 = time()
        quote_usd = self.portfolio.wallets[quote_ticker + '-' + value_ticker].product
        base_usd = self.portfolio.wallets[base_ticker + '-' + value_ticker].product
        ...
        # p3 * p2 * amt / p1
        print(time() - t0)
        return quote_to_base_value, base_to_quote_value
